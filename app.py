...
def handle_write_task(message_text):
    ...
    5.  不要在 JSON 以外添加任何 'json' 標記或任何說明文字。只輸出純 JSON 物件。
    """
    try:
        response = gemini_model.generate_content(prompt)
        raw_text = response.text
        app.logger.info(f"Gemini 原始回傳: {raw_text}")
        
        # 清理 JSON 字串
        json_string = clean_json_response(raw_text)
        app.logger.info(f"清理後的 JSON: {json_string}")
        ...
        app.logger.error(f"JSON 解析失敗: {je}")
        app.logger.error(f"原始回傳: {raw_text if 'raw_text' in locals() else 'N/A'}")
        raise Exception(f"JSON 解析失敗,請檢查 Gemini 回傳格式。")
    except Exception as e:
        app.logger.error(f"Gemini API 失敗: {e}")
        raise Exception(f"Gemini 解析資料失敗,請檢查 Prompt 或 API 金鑰。")
    
    try:
        new_row = [
            task_data.get('project_name', '未指定'),
            task_data.get('task_name', '未提取'),
            task_data.get('assignee', '未指定'),
            "待辦",
            "中",
            app.config.get('TODAYS_DATE', ''),
...
def parse_query_to_filters(message_text):
    ...
    try:
        response = gemini_model.generate_content(prompt)
        raw_text = response.text
        app.logger.info(f"V5 查詢 - 階段 1 原始回傳: {raw_text}")
        
        # 清理 JSON 字串
        json_string = clean_json_response(raw_text)
        app.logger.info(f"V5 查詢 - 階段 1 清理後: {json_string}")
